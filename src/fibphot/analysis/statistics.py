...
StatName = Literal[
    "count", "mean", "std", "sem", "median", "min", "max", "q25", "q75"
]
...
def nan_sem(
    x: np.ndarray, axis: int | None = None, ddof: int = 1
) -> np.ndarray:
...
def nan_stats(
    x: np.ndarray, *, axis: int | None = 0, ddof: int = 1
) -> dict[str, np.ndarray]:
    ...
    *,
    ...
    if values is None:
        ...
        values = [
            ...
            if c not in excluded and not c.endswith("_index")
        ]
    ...
    iterator = (
        df.groupby(group_cols, dropna=False) if group_cols else [((), df)]
    )
