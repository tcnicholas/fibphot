...
from .events import EventAlignedTraces, align_to_events, event_aligned_summary_dataframe, event_aligned_traces_dataframe, select_events_by_spacing
...
def _normalise_channels(channels: Sequence[str] | str | None, state: PhotometryState) -> tuple[str, ...]:
    if channels is None or channels == "all":
        return tuple(state.channel_names)
    if isinstance(channels, str):
        return tuple(c.strip().lower() for c in channels.split(",") if c.strip())
...
def _filter_detector_params(factory: Any, params: Mapping[str, Any]) -> dict[str, Any]:
...
def _detector_from_config(*, detector: DetectorKind, event_signal: str, detector_params: Mapping[str, Any] | None, window: AnalysisWindow | None):
    ...
    if detector == "template":
        ...
        return PeaksByTemplate(**_filter_detector_params(PeaksByTemplate, params))
    if detector == "peaks":
...
def result_to_event_aligned(result: AnalysisResult) -> EventAlignedTraces:
    ...
    required = {"time_relative_s", "aligned_traces", "aligned_channel_names", "event_time_s"}
    missing = required.difference(arrays)
    if missing:
        raise ValueError(f"Result does not contain aligned traces. Missing: {sorted(missing)}")
    return EventAlignedTraces(
        ...
        channel_names=tuple(str(x) for x in np.asarray(arrays["aligned_channel_names"]).tolist()),
        event_times_s=np.asarray(arrays["event_time_s"], dtype=float),
        event_metadata=tuple({"event_i": int(i)} for i in range(len(np.asarray(arrays["event_time_s"])))),
    )


def peak_triggered_summary_dataframe(result: AnalysisResult):
    aligned = result_to_event_aligned(result)
    df = event_aligned_summary_dataframe(aligned, statistics=("mean", "std", "sem", "median", "q25", "q75"))
    df.insert(0, "analysis", result.name)
    df.insert(1, "event_signal", result.channel)
    return df


def peak_triggered_traces_dataframe(result: AnalysisResult, *, max_events: int | None = None):
    aligned = result_to_event_aligned(result)
...
class PeakTriggeredAverage:
    ...
    def __call__(self, state: PhotometryState) -> AnalysisResult:
        ...
        detector = _detector_from_config(detector=self.detector, event_signal=self.event_signal, detector_params=self.detector_params, window=self.window)
        peak_res = detector(state)
        if not peak_res.arrays or "x" not in peak_res.arrays:
            return AnalysisResult(self.name, self.event_signal, self.window, self._params(), {"n_events": 0.0, "n_channels": float(len(channels))}, {}, "No peaks were detected for alignment.")
        ...
        if self.exclude_times:
            windows = [(float(a) - self.exclude_margin_s, float(b) + self.exclude_margin_s) for a, b in self.exclude_times]
            if self.exclude_mode == "peak_time":
                ...
                raise ValueError("exclude_mode must be 'peak_time' or 'window_overlap'.")
        if self.min_previous_interval_s is not None or self.min_next_interval_s is not None:
            keep &= select_events_by_spacing(event_times, min_previous_interval_s=self.min_previous_interval_s, min_next_interval_s=self.min_next_interval_s)
        event_times_kept = event_times[keep]
        event_indices_kept = event_indices[keep]
        meta = tuple({"source_peak_i": int(i)} for i in event_indices_kept.tolist())
        aligned0 = align_to_events(state, event_times_kept, t_before=float(self.t_before_s), t_after=float(self.t_after_s), channels=channels, dt=self.dt, target_fs=self.target_fs)
        aligned = EventAlignedTraces(aligned0.time_relative_s, aligned0.data, aligned0.channel_names, aligned0.event_times_s, meta)
        data = aligned.data.copy()
        if self.baseline_window_s is not None and data.size:
            lo, hi = self.baseline_window_s
            m = (aligned.time_relative_s >= float(lo)) & (aligned.time_relative_s <= float(hi))
            if np.any(m):
                data = data - np.nanmean(data[:, :, m], axis=2, keepdims=True)
                aligned = EventAlignedTraces(aligned.time_relative_s, data, aligned.channel_names, aligned.event_times_s, aligned.event_metadata)
        ...
        for ci, channel in enumerate(aligned.channel_names):
            for stat_name in ("mean", "std", "sem", "median", "q25", "q75", "count"):
                arrays[f"{stat_name}__{channel}"] = np.asarray(stats[stat_name][ci])
        metrics = {"n_events": float(aligned.n_events), "n_detected_events": float(event_times.size), "n_channels": float(len(aligned.channel_names)), "t_before_s": float(self.t_before_s), "t_after_s": float(self.t_after_s)}
        for ci, channel in enumerate(aligned.channel_names):
            vals = data[:, ci, :]
            metrics[f"{channel}_mean_peak_aligned_mean"] = float(np.nanmean(vals)) if vals.size else np.nan
            metrics[f"{channel}_mean_peak_aligned_max"] = float(np.nanmax(stats["mean"][ci])) if vals.size else np.nan
            metrics[f"{channel}_mean_peak_aligned_min"] = float(np.nanmin(stats["mean"][ci])) if vals.size else np.nan
        return AnalysisResult(self.name, self.event_signal, self.window, self._params(), metrics, arrays, "peak/event-triggered aligned traces and session average")

    def _params(self) -> dict[str, Any]:
        return {
            "event_signal": self.event_signal,
            "channels": list(self.channels) if isinstance(self.channels, (list, tuple)) else self.channels,
