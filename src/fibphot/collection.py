...
from .analysis.aggregate import AlignedSignals, TraceStatistics, align_collection_signals, mean_state_from_aligned, trace_statistics_from_aligned
...
def _source_metadata(path: Path, *, base_path: Path | None = None) -> dict[str, Any]:
    p = path.resolve()
    meta: dict[str, Any] = {
        "source_path": str(p),
        "source_name": p.name,
...
class BatchReport:
    ...
    def grouped_metrics(self, groupby: str | Sequence[str] | None = None, *, values: Sequence[str] | None = None):
        from .analysis.statistics import grouped_numeric_summary
        return grouped_numeric_summary(self.metrics_dataframe(), groupby=groupby, values=values)

    def grouped_arrays(self, groupby: str | Sequence[str] | None = None, *, values: Sequence[str] | None = None):
        from .analysis.statistics import grouped_numeric_summary
        return grouped_numeric_summary(self.arrays_dataframe(), groupby=groupby, values=values)
...
class PhotometryCollection:
    ...
    @classmethod
    def from_iterable(cls, states: Iterable[PhotometryState]) -> PhotometryCollection:
    ...
    def from_glob(
        ...
        *,
        ...
        return cls.from_paths(paths, reader=reader, metadata_fn=metadata_fn, base_path=base, sort=sort)
    ...
    def from_directory(
        ...
        *,
        ...
        base = Path(base_path)
        return cls.from_paths(base.glob(pattern), reader=reader, metadata_fn=metadata_fn, base_path=base, sort=sort)
    ...
    def __getitem__(self, item: int | slice) -> PhotometryState | PhotometryCollection:
    ...
    def pipe(self, *stages: object, history: HistoryPolicy = "raw") -> PhotometryCollection:
        return PhotometryCollection(states=tuple(s.pipe(*stages, history=history) for s in self.states))
        ...
        for i, state in enumerate(self.states):
            ...
            try:
                for st in stages:
                    ...
                    current = run(current, history=history) if callable(run) else st(current)
                ok.append(current)
                ...
                failures.append(
                    BatchFailure(
                        ...
                        source_path=str(state.metadata.get("source_path")) if state.metadata.get("source_path") else None,
                ...
                if not continue_on_error:
                    raise
        return BatchResult(successful=PhotometryCollection(ok), failed=tuple(failures))

    def analyse(self, *analyses: object, continue_on_error: bool = True) -> BatchReport:
    ...
    def with_tags_from_table(
        ...
        *,
        ...
        return self.with_tags(read_tag_table(path), subject_getter=subject_getter, overwrite=overwrite)
    ...
    def with_tags(
        ...
        *,
        ...
        tagged = tuple(apply_tags(s, table, subject_getter=subject_getter, overwrite=overwrite) for s in self.states)
    ...
    def with_metadata_table(
        ...
        *,
        ...
        p = Path(path)
        table = pd.read_excel(p) if p.suffix.lower() in {".xlsx", ".xls"} else pd.read_csv(p)
        if on not in table.columns:
            raise ValueError(f"Metadata table does not contain match column {on!r}.")
        ...
        for state in self.states:
            ...
            meta = dict(state.metadata); tags = dict(state.tags)
    ...
    def filter(self, predicate: Callable[[PhotometryState], bool] | None = None, **criteria: Any) -> PhotometryCollection:
        """Filter by metadata or tags."""
        def match_value(value: Any, criterion: Any) -> bool:
            if callable(criterion):
                return bool(criterion(value))
            if isinstance(criterion, (set, list, tuple)) and not isinstance(criterion, str):
        ...
        def ok(s: PhotometryState) -> bool:
            if predicate is not None and not predicate(s):
                return False
            return all(match_value(metadata_value(s, k, None), v) for k, v in criteria.items())

        return PhotometryCollection(states=tuple(s for s in self.states if ok(s)))

    def groupby(self, key: str | Sequence[str] | Callable[[PhotometryState], str]) -> dict[str, PhotometryCollection]:
        ...
        return {k: PhotometryCollection(states=tuple(v)) for k, v in groups.items()}
    ...
    def sort_by(self, *keys: str) -> PhotometryCollection:
        ...
        return PhotometryCollection(states=tuple(sorted(self.states, key=sort_key)))

    def map(self, fn: Callable[[PhotometryState], PhotometryState]) -> PhotometryCollection:
    ...
    def summary_table(self):
        ...
        for i, s in enumerate(self.states):
            row: dict[str, Any] = {
                ...
                "duration_s": float(s.time_seconds[-1] - s.time_seconds[0]) if s.n_samples >= 2 else np.nan,
                "channels": ",".join(s.channel_names),
                "n_stages": len(s.summary),
            }
            for k, v in s.metadata.items():
                if k not in row and isinstance(v, (str, int, float, bool, type(None))):
                    row[k] = v
    ...
    def grouped_mean(
        ...
        *,
        ...
        return {
            key: group.mean(channels=channels, align=align, time_ref=time_ref, dt=dt, target_fs=target_fs, interpolation=interpolation, fill=fill, name=f"group_mean:{key}")
            for key, group in self.groupby(by).items()
        }

    def to_h5(self, path: Path | str, *, compression: str | None = "gzip", compression_opts: int = 4) -> None:
        from .io.h5 import save_collection_h5
        save_collection_h5(self, path, compression=compression, compression_opts=compression_opts)
    ...
    def __repr__(self) -> str:
        ...
        info["stages_minmax"] = ReprText(f"({int(depths.min())}, {int(depths.max())})")
        fs = np.array([s.sampling_rate for s in self.states], dtype=float)
        fs = fs[np.isfinite(fs)]
        if fs.size:
            info["fs_hz_med"] = sig(float(np.median(fs)), 4)
            info["fs_hz_minmax"] = ReprText(f"({sig(float(fs.min()), 4)}, {sig(float(fs.max()), 4)})")
        dur = np.array([float(s.time_seconds[-1] - s.time_seconds[0]) if s.n_samples >= 2 else 0.0 for s in self.states], dtype=float)
        info["duration_s_med"] = sig(float(np.median(dur)), 4)
        info["duration_s_minmax"] = ReprText(f"({sig(float(dur.min()), 4)}, {sig(float(dur.max()), 4)})")
        ...
        return uniform_repr("PhotometryCollection", **info, indent_width=4, max_width=88)
